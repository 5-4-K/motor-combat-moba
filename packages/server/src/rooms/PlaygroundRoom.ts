  ...
  private tick(): void {
    ...
    if (debug && this.state.tick % 6 === 0) {
      this.broadcast(MSG_PLAYGROUND_BOT_DEBUG, {
        ...
        // `debug.plan` is `undefined` only before the bot's first recompute window; 0/0/0 reads
        // correctly as "no plan yet" on the overlay just like `firedSlot: -1` does above.
