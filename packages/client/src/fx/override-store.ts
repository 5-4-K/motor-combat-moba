 ...
 *
 * Not a resolver built once over a captured map: the panel edits the map while the `FxLayer` that
 * will render the next burst already exists, so a resolver that captured the map at construction
 * would render the state the panel opened with until the next arena restart.
 */
