...
export const WEAPON_GLOW_STYLES: Partial<Record<WeaponId, GlowStyle>> = {
  magmablast: {
    ...
    // Out to 2.5x the 12-unit hitbox — 30 units, exactly half a car length (60 u). Continues the
...
export interface DrawBand {
   ...
   * anything whose style authored none — including every solid glow band. Halo bands author a
   * value in (0, 1).
   */
  alpha: number;
}
 ...
 * at about `1.5 / sqrt(D)` Hz with D the flame's width in metres; at this game's scale (a 60-unit
 * car is about 4.5 m, so 13.3 units per metre) `afterburner`'s roughly 1.6 m wide plume puffs near
 * 1.2 Hz. The small roll-up vortices inside the luminous flame of the same flames were measured at
...
/**
 * Half the bracket's side, world units. Larger than a car hull's half-diagonal (36 units for the
 * 60 x 40 hull) so the bracket frames the car instead of being drawn across it. Scaled 34 -> 42.5
 * with the 2026-09-16 hull resize, keeping its old proportion to the car.
 */
export const LOCK_BRACKET_HALF = 42.5;

/** How far each arm runs from its corner. Kept well under the side, so the corners never join. */
export const LOCK_BRACKET_ARM = 13.75;
