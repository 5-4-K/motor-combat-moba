...
/**
 * The car's additive underglow as nested rings, widest first — the same stacking softness
 * `shadowBandsFor` builds, and equal alphas for exactly the same reason. See that function's comment:
 * the bands overlap, so the centre lands near `glowAlpha` and the outer edge at a fraction of it,
 * and ramping the alphas as well double-counts the gradient.
 *
 * **This is the one layer here that is not about volume.** The rest of this module gives a car a lit
 * side, a shaded side and a shadow — which reads beautifully on the near-black generated asphalt it
 * was tuned against, and nearly disappears on a mid-grey floor sprite. The glow is what separates a
 * car from ground of a similar value, by making the car the brightest thing near itself.
 *
 * Radial and centred, so unlike every other function in this file it takes NO rotation — there is
 * nothing to un-turn, and a halo that changed shape with heading would leak the car's facing to
 * anyone who learned to read it.
 *
 * Returns `[]` when the glow is off, so the caller skips the fills rather than stacking transparent
 * ones — and so zeroing it restores the pre-glow drawing exactly.
 */
export function glowBandsFor(look: CarLook): ShadowBand[] {
...
/**
 * The colour a car's underglow is drawn in: its own player colour, lifted toward white by
 * `glowColorMix`.
 *
 * Kept as the player's colour rather than a neutral light on purpose. The glow is the brightest
 * thing attached to a car, so it is also the most legible statement of whose car it is — mixing it
 * to white buys a little more brightness and throws that away. `GLOW_DEPTH`'s comment makes the same
 * argument from the other end: additive light is kept OFF a chassis precisely so it cannot drain the
 * player colour.
 */
export function glowColorFor(fill: number, look: CarLook): number {
  return mixColor(fill, 0xffffff, Math.min(1, Math.max(0, look.glowColorMix)));
}
