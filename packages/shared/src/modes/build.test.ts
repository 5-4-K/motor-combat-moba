...
describe("assembleModeConfig", () => {
  it("derives the five artifacts the sim reads", () => {
    const config = assembleModeConfig(GameMode.FFA_LAST_STANDING, BRAWL_TABLES);
    expect(Object.keys(config.derived.weaponTicks)).toEqual(Object.keys(BRAWL_TABLES.weapons));
    expect(Object.keys(config.derived.chassisDrive)).toEqual(Object.keys(BRAWL_TABLES.cars));
  ...
  it("gives two bundles built from the same tables independent objects", () => {
    ...
    const b = assembleModeConfig(GameMode.FFA_DEATHMATCH, BRAWL_TABLES);
