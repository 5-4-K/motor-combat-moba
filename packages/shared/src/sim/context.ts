 ...
 * `RamCar.defenceMult` (the same `ramDefence` channel, dividing the shove a ram lands on this car in
 * `shoveOf`) or `ram-bridge.ts`'s `ramDefenceFor` (feeding a slam's `applyImpulse` inertia term) —
 * this function has access to neither, since ordinary driving has no `Modifiers` map in scope the way
 * `serverTick`'s ram-adjacent code does. So a `ramDefence` status buff changes how easily a car is
 * rammed and how far a slam throws it, but NOT how much ground it gives up in the ordinary, non-ram
