...
The point is that the icon in the HUD slot and the shot crossing the arena read as the same weapon.
The slot is where a player learns "predator = ember red"; they should then recognise it in flight
without being told. Nothing enforces this — `import-weapon-icon.mjs` never inspects an image's
palette, so a blue `predator` icon over red `predator` shots imports silently and looks like a bug in
