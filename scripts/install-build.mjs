...
/**
 * Exactly what `scripts/build-release.mjs` writes into `dist-release/motor-combat-moba/`, and so
 * exactly what this script owns in the target folder. Everything else there — `node_modules`, a
 * `.env`, logs, whatever the user keeps beside the game — is left alone.
 *
...
export async function main(argv = process.argv.slice(2)) {
  ...
  console.log(`Start it with ${startScript} in that folder, then open http://localhost:2567\n`);
}
