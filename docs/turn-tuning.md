...
|---|---|---|---|---|---|---|---|---|---|
| `coastHalfLifeSeconds` — coast half-life (s) | 1.0 | 1.2 | 1.5 | 1.5 | 1.5 | 1.2 | 1.2 | 1.0 | 1.5 |
...
|---|---|---|---|---|---|---|---|---|---|---|
| **Turn rate** | `baseTurnRate + handling × turnRatePerRating` | 7.11 rad/s | **8.19 rad/s** | 6.3 rad/s | 6.3 rad/s | 6.3 rad/s | 8.19 rad/s | 8.19 rad/s | 7.11 rad/s | 6.3 rad/s |
| — in degrees | × 180/π | 407.4°/s | 469.3°/s | 361.0°/s | 361.0°/s | 361.0°/s | 469.3°/s | 469.3°/s | 407.4°/s | 361.0°/s |
| — per tick | ÷ `TICK_RATE_HZ` (30) | 0.237 rad | 0.273 rad | 0.21 rad | 0.21 rad | 0.21 rad | 0.273 rad | 0.273 rad | 0.237 rad | 0.21 rad |
| — degrees per tick | ″ | 13.58° | 15.64° | 12.03° | 12.03° | 12.03° | 15.64° | 15.64° | 13.58° | 12.03° |
| **Turn rate at rest** | `turnRate × stopTurnRatio` | 3.555 rad/s | 4.095 rad/s | 3.15 rad/s | 3.15 rad/s | 3.15 rad/s | 4.095 rad/s | 4.095 rad/s | 3.555 rad/s | 3.15 rad/s |
| — in degrees | ″ | 203.7°/s | 234.6°/s | 180.5°/s | 180.5°/s | 180.5°/s | 234.6°/s | 234.6°/s | 203.7°/s | 180.5°/s |
...
| Reverse top speed | `× reverseSpeedRatio` | 103.1 u/s | 122.9 u/s | 88.3 u/s | 88.3 u/s | 88.3 u/s | 122.9 u/s | 122.9 u/s | 103.1 u/s | 88.3 u/s |
| **Turn radius** | `topSpeed / turnRate` | 22.3 u | 23.1 u | **21.6 u** | 21.6 u | 21.6 u | 23.1 u | 23.1 u | 22.3 u | 21.6 u |
| Reverse turn radius | `reverseSpeed / turnRate` | 14.5 u | 15 u | 14 u | 14 u | 14 u | 15 u | 15 u | 14.5 u | 14 u |
| 180° while moving | `π / turnRate` | 0.44 s | 0.38 s | 0.5 s | 0.5 s | 0.5 s | 0.38 s | 0.38 s | 0.44 s | 0.5 s |
| 360° while moving | `2π / turnRate` | 0.88 s | 0.77 s | 1 s | 1 s | 1 s | 0.77 s | 0.77 s | 0.88 s | 1 s |
| 180° from standstill | `π / turnRateAtStop` | 0.88 s | 0.77 s | 1 s | 1 s | 1 s | 0.77 s | 0.77 s | 0.88 s | 1 s |
| Rate while reeling | `turnRate × reeling.turnRate (0.4)` | 2.844 rad/s | 3.276 rad/s | 2.52 rad/s | 2.52 rad/s | 2.52 rad/s | 3.276 rad/s | 3.276 rad/s | 2.844 rad/s | 2.52 rad/s |
...
rework — `PlayerState` lost its `authority` field, and `ram-bridge.ts` dropped `knock.authority` on
the floor — so it was printing an arithmetically correct number for a steering cap that applied to
...
```
derived "Turn rate" / mirage: page says 8.19, config gives 8.5
```
...
| `STATUS_TABLE` | any status carrying a `turnRate` modifier — `reeling`'s (0.4) has the "Rate while reeling" row in the derived table; a second such status needs a row of its own. `overheated` used to be the example here and lost its `turnRate` in the 2026-09-01 status overhaul |
