...
You are writing ONE implementation plan for the Motor Combat MOBA repo. **Every path below is relative to the repository root** — an earlier revision of this brief hardcoded one machine's checkout (`/home/user/motor-combat-moba`) and one machine's plugin cache, both of which are wrong everywhere else; if you find an absolute path in this file, it is a bug in the file. You write a markdown file and nothing else: do not edit code, do not commit, do not read docs/ideas/ or docs/invariants/.

READ FIRST, in this order:
1. The **`superpowers:writing-plans`** skill — invoke it by name through the Skill tool rather than reading it off a path, because the plugin cache lives somewhere different on every machine. Follow its format exactly: the header block, Global Constraints, File Structure table, tasks with Files / Interfaces / checkbox steps, real code in every code step, tests before implementation, a commit step per task, and the Self-review section at the end. The "No Placeholders" rules are hard rules.
2. `CLAUDE.md` at the repo root and `packages/{shared,server,client}/CLAUDE.md` — repo rules (shared dist gotcha, root `npm test`, playtest "say so loudly" rule, never create probes, docs/turn-tuning.md test, manual page fingerprint, no magic numbers in logic).
