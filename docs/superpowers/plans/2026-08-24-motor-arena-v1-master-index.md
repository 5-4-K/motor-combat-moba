...
| P4 | In progress | 2026-08-24 | — | agent, automated: `npm run test --workspaces` (242), `npm run build --workspaces`. agent, live two-tab browser runs at 0ms and at `SIM_LATENCY_MS=80 SIM_JITTER_MS=20` (ack RTT measured 92ms avg, 53-116ms, 63ms spread — injector confirmed active): movement, wall/obstacle collision at exact geometry, per-car speed caps matching CAR_TABLE, restitution, remote interpolation (rendered moved on 72% of frames vs server 9%), match-2 prediction (12u lead), prediction lead under latency bounded (avg 15.8u, max 35.9u, pending buffer max 3), 566u injected error snapped to 0 with no oscillation, ack monotonic over 340 patches (0 regressions), all three car shapes distinct (rect 15 draw ops / hexagon 33 / oval 111) with distinct COLOR_TABLE fills | **OUTSTANDING before Done:** hands-on feel check by a human — camLerp responsiveness, and whether the documented wall-grind and 30.6-degree bounce sign flip are acceptable in play. All mechanical validation is complete; only subjective feel remains. |
| P5 | Not started | — | — | — | |
