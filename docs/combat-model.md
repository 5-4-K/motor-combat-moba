...
Every point of hp loss already has a known attacker, because ramming deals no damage (see Ramming
above) and status damage-over-time already carries `ActiveStatus.sourceSessionId`. `CombatPlayer`
carries `lastDamagerSessionId`, stamped by `damage()` in `sim/combat.ts` — the sim's one hp/`alive`
writer — from either the shot's `ownerSessionId` or the pulse's `sourceSessionId`. It is **server-only,
