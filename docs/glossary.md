...
| **Aura** | A beam with a `disc` hitbox anchored at the car's centre: a field around a car rather than a line of fire. Passes through walls; drawn as a ring. **Dormant machinery** since the 2026-09-01 roster cutover — `magmablast` (then `shockwave`) was the one shipped aura and no longer is, so no `WEAPON_TABLE` row uses a `disc` hitbox today. |
...
| **Bullseye** | Type 1. The light, precise skirmisher: lowest hp and mass, the roster's longest reach, the lowest turn *rate*. Carries `magmablast`, `pepperbox`, `lance`, and applies no status at all. |
