...
| **Bullseye** | Type 1. The light, precise skirmisher: lowest hp and mass, the roster's longest reach, a middling turn rate (`speed`/`handling` both 65, between Mirage's 85 and Bastion's 50 since the 2026-09-02 speed/handling unification). Carries `predator`, `pepperbox`, `lance`, and applies no status at all. |
...
| **Bastion** | Type 3. The tank: slowest, heaviest, highest hp, and — since the 2026-09-02 speed/handling unification — the *lowest* turn rate in the game (`speed`/`handling` both 50, down from a rating that used to lead the roster). Its turn *radius* still edges out the other two (51 u, tightest, vs Bullseye's 53 u and Mirage's 55 u) purely because its top speed is also lowest; the tank identity now rests on hp and mass, not a handling edge. Carries `thumper`, `roadblock`, `wildcharge`; its hard CC (`roadblock`'s `stunned`, and the wall-slam stun off `wildcharge`) now shares the CC role with Mirage's `thunderclap`, which also stuns on hit. |
